...
\begin{onlysolution}
  ...
  68
  \end{source}
  При сложении типов \code{Int32} и \code{Char} произойдёт конвертация \code{Char} в \code{Int32}. Таким образом, во всех трёх случаях результат будет представлять собой код символа \code{'A'} (65), увеличенный на 3.
\end{onlysolution}
\end{defproblem}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{onlyproblem}
\begin{onlysolution}
  Можно заинтернировать строку \code{"Hello"}, а затем через unsafe-код добраться до соответствующего участка памяти и изменить целевое значение:  
  \begin{source}
  var s = "Hello";
  string.Intern(s);
  unsafe
